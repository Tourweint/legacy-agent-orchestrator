...
## 当前状态：阶段 0–8 全部完成（2026-09-25）+ P1 安全与鲁棒性修复（2026-09-26）

L1–L5 五层与混沌层均已落地，对外入口已开放（8090），并经混沌实验 D0–D6 验证（留档 `docs/实验/`）。2026-09-26 完成 P1 安全与鲁棒性修复（6 项，见 [变更记录](../docs/变更记录/2026-09-26-P1安全与鲁棒性修复.md)）：

| P1 修复项 | 内容 |
|---|---|
| 异步异常收敛 | `runTaskSafely()` 包装后台 Promise，任意异常写入证据链并置 UNRESOLVED 终态，消除任务悬空（I3） |
| SSE 重放竞态 | 改为"先订阅→取快照 seq→重放历史"的快照点分界模式，消除事件丢失 |
| 请求体上限 | `readBody()` 64KB 上限，超限返回 413；7 项字段长度限制 |
| 严格 UTC 校验 | `parseUtcInstant()` 只接受带时区的 ISO 字符串，无时区返回 400 |
| 任务存储治理 | TaskStore 重写：最大任务数 1000、终态 TTL 30 分钟、挂起 TTL 1 小时、定期清理 |
| 安全边界 | CORS 从 `*` 改为显式来源 `http://localhost:5173`；默认绑定 `127.0.0.1`（`ORCH_BIND_HOST` 可覆盖） |
...
npm start         # 启动引擎（8090；前置：存量系统 8080 与 Redis 就绪；凭证走环境变量；默认绑定 127.0.0.1，CORS 显式来源 http://localhost:5173）
...
3. **时间只许一处解析**：出参时间字符串的 UTC 解释、入参 Z 构造、三段区间换算全部在 `canonical/time.js`；业务代码里出现"本地时区"即为违规（第 06 章 §6.3 规格 5）。**入参必须带时区（`Z` 或偏移），无时区字符串在接入层即被拒绝（400），不得进入业务逻辑。**
...
8. **后台任务必须有终态**（I3）：所有异步 Promise 必须经 `runTaskSafely()` 包装，任意异常必须写入证据链并生成终态事件，不得悬空。
9. **请求体有上限**：接入层 `readBody()` 限制 64KB，超限返回 413；单字段长度也有限制（见 `constants.yaml`）。不得无限累积请求数据。
10. **任务存储有治理**：TaskStore 有最大任务数、终态 TTL、挂起 TTL 和定期清理，不得无限增长。
11. **写超时不重试**：写请求超时绝不重试（可能已写成功），必须先走幂等查证路径收敛结论。
12. **安全边界**：默认绑定 `127.0.0.1`，CORS 使用显式来源而非 `*`；不得将 8090 直接暴露公网。
